...
@Component({
  selector: 'app-forecasters',
  templateUrl: './forecasters.component.html',
  styleUrls: ['./forecasters.component.scss']
})
export class ForecastersComponent implements OnInit, AfterViewInit, OnDestroy {

  title = 'Forecasters management';
  ...
  ngOnInit() {
    ...
    this.socketEvent = this.forecastersService.socketEvent;
  }
  ...
  addRow(modal: NgbActiveModal): void {
    ...
    if (this.validateForecaster().length <= 0) {
      this.forecastersService.addForecaster(this.forecaster).pipe(takeUntil(this.isForecastersDead$)).subscribe((response) => {
        this.forecaster = { _id: '-1', name: '', authServer: '', dataServer: '', login: '', password: '' };
        ...
        tempLog.severity = 1;
        tempLog.message = `Added the forecaster ${response.data.name}`;
  ...
  saveEditRow(modal: NgbActiveModal) {
    // save in db
    if (this.validateForecaster().length <= 0) {
      this.forecastersService.updateForecaster(this.forecaster).pipe(takeUntil(this.isForecastersDead$)).subscribe((response) => {
        this.forecaster = { _id: '-1', name: '', authServer: '', dataServer: '', login: '', password: '' };
        modal.close('Close click');
        const tempLog = Object.assign({}, this.log);
        const user = this.userSharingService.currentUser;
        ...
        tempLog.date = Date.now();
        tempLog.severity = 1;
        tempLog.message = `Updated the forecaster ${response.data.name}`;
        this.logsService.addLog(tempLog).pipe(takeUntil(this.isLogsDead$)).subscribe();
      }, (error) => {
        this.errorModal = true;
        this.errorMessageModal = 'An error occurred on the server side.';
  ...
  deleteRow(modal: NgbActiveModal) {
    ...
    if (this.forecaster._id !== undefined || this.forecaster._id !== null || this.forecaster._id !== '') {
      this.forecastersService.deleteForecaster(this.forecaster._id).pipe(takeUntil(this.isForecastersDead$)).subscribe((response) => {
        if (response.success === true) {
          ...
          tempLog.date = Date.now();
          tempLog.severity = 2;
          tempLog.message = `Deleted the forecaster ${forecasterName}`;
          this.logsService.addLog(tempLog).pipe(takeUntil(this.isLogsDead$)).subscribe();
        } else {
